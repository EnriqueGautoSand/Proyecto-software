...
var url = "http://"+location.host+Flask.url_for("Clienteapi.datoscliente_convertir")
...
var url = "http://"+location.host+Flask.url_for("Pedidoapi.datospedido")
var data = { hashpedido:window.location.pathname.split("/")[window.location.pathname.split("/").length-1]};
var cliente = {}
	// statements
...
function conectarVentaapi(jsonventa){
	var url = "http://"+location.host+Flask.url_for("VentasApi.convertir_pedido_venta")//"http://localhost:8080/api/v1/ventasapi/realizarventa/"
var data = jsonventa;
try {
	fetch(url, {
  method: 'POST', // or 'PUT'
